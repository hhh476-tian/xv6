...
e1000_transmit(struct mbuf *m)
{
  ...
  // find next index in the transmit ring
  int index = regs[E1000_TDT] % TX_RING_SIZE;

  // return error if previous packet not finished yet
  ...
  // fill in new descriptor info
  memset(&tx_ring[index], 0, sizeof(tx_ring[index]));
  tx_ring[index].addr = (uint64)m->head;
  tx_ring[index].length = m->len;
  tx_ring[index].cmd = E1000_TXD_CMD_RS | E1000_TXD_CMD_EOP;

  // save a pointer to mbuf for free later
  tx_mbufs[index] = m;
...
e1000_recv(void)
{
  ...
  while(1) {
    ...
    // send the mbuf to network stack
    ...
    // create a new mbuf for the receive ring
